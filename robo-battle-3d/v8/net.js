/**
 * ROBO BATTLE 3D v7 - P2P ネットワーク基盤 (スター型・最大4人)
 *
 * 構成:
 *   - Firebase Realtime Database でシグナリング(部屋コード/Offer/Answer/ICE 交換)
 *   - WebRTC DataChannel で低遅延の状態同期
 *   - スター型: ホスト 1 + ゲスト 最大3 = 計4人。ホストが各ゲストと 1 本ずつ接続(権威)。
 *
 * v6 時代の online-mode.js(1v1)を多人数へ一般化したもの。Firebase の関数は
 * firebase-config.js が window.firebase* / window.initFirebase に出す。
 *
 * 使い方(概略):
 *   const net = new NetHub();
 *   net.on('lobby', s => ...).on('open', id => ...).on('data', (id,m) => ...).on('close', id => ...);
 *   const code = await net.createRoom('HOST名');   // ホスト
 *   await net.joinRoom(code, 'ゲスト名');           // ゲスト
 *   net.broadcast({...}) // ホスト→全ゲスト / net.toHost({...}) // ゲスト→ホスト
 */
...
export const MAX_PLAYERS = 4; // ホスト + ゲスト3
...
export class NetHub {
  ...
  async joinRoom(code, name) {
    ...
    const snap = await FB.get(`rooms/${this.code}`);
    ...
    await FB.upd(`rooms/${this.code}/players/${this.selfId}`, { name: this.name, ready: false, joinedAt: Date.now() });
    this._guestListen();
  }
  ...
  _hostListen() {
    ...
    this.unsub.push(FB.on(`rooms/${this.code}/players`, (snap) => {
    ...
    // 部屋メタ(state など)も監視
    this.unsub.push(FB.on(`rooms/${this.code}`, (s) => { const r = s.val(); if (r) this._room = r; }));
  }
  ...
  async _hostConnectTo(gid, info) {
    ...
    const base = `rooms/${this.code}/sig/${gid}`;
  ...
  // ================= ゲスト側 =================
  _guestListen() {
    const base = `rooms/${this.code}/sig/${this.selfId}`;
    const pc = new RTCPeerConnection({ iceServers: ICE });
    ...
    // 部屋(ロビー/開始)監視
    this.unsub.push(FB.on(`rooms/${this.code}`, (s) => {
      const room = s.val();
  ...
  async setReady(v) {
    const val = v !== false;
    const path = this.isHost ? `rooms/${this.code}/host` : `rooms/${this.code}/players/${this.selfId}`;
    await FB.upd(path, { ready: val });
  }
  async startMatch() { if (this.isHost) await FB.upd(`rooms/${this.code}`, { state: 'playing' }); }
  ...
  async leave() {
    ...
    if (this.code) {
      try {
        if (this.isHost) await FB.rm(`rooms/${this.code}`);
        else await FB.rm(`rooms/${this.code}/players/${this.selfId}`);
      } catch (e) {}
    }
    this.code = null;
  }
