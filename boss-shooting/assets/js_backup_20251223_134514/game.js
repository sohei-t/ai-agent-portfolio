...
class Game {
    ...
    spawnPowerup(x, y) {
        if (typeof Powerup !== 'undefined') {
            // 拡張された武器タイプを含むリスト（特殊武器を追加）
            const types = [
                'weapon', 'weapon', 'weapon',  // 通常武器レベルアップ
                'weapon_spread', 'weapon_laser', 'weapon_homing', 'weapon_wave',  // 新武器
                'life', 'bomb', 'shield', 'power', 'speed',
                'option', 'option',  // オプション
                'summon_phoenix', 'summon_dragon', 'summon_thunder',  // 召喚獣
                'mega_laser', 'combine'  // 特殊攻撃
            ];
