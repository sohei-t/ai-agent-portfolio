...
const stageData = {
    ...
    10: {
        ...
        scrollSpeed: 2,
        enemyWaves: [
            { time: 100, type: 'mixed', count: 10, formation: 'chaos' },
            { time: 500, type: 'tank', count: 5, formation: 'wall' },
            { time: 900, type: 'all', count: 15, formation: 'ultimate' },
        ],
...
function spawnMidBoss(game, type) {
    ...
    if (typeof Enemy !== 'undefined') {
        ...
        midBoss.width = 120;  // 64→120（さらに大型化）
        midBoss.height = 120;  // 64→120（さらに大型化）
        midBoss.hp = 80;  // 20→80（4倍）
        midBoss.maxHp = 80;  // 20→80（4倍）
        midBoss.scoreValue = 7500;
        midBoss.movePattern = 'hover';
        midBoss.attackPattern = 'spread';
        midBoss.color = '#ff00ff';
        midBoss.attackInterval = 40;  // 攻撃頻度をさらに上げる
        midBoss.type = 'mid-boss';  // タイプを明示的に設定
        midBoss.speed = 1.5;  // 移動速度も上げる

        // 特別な攻撃パターンを追加
        midBoss.originalUpdate = midBoss.update;
        midBoss.update = function(dt) {
            midBoss.originalUpdate.call(this, dt);

            // HPが半分以下になったら攻撃頻度を上げる
            if (this.hp < this.maxHp / 2 && this.attackInterval > 25) {
                this.attackInterval = 25;
                this.attackPattern = Math.random() > 0.5 ? 'spread' : 'aimed';
            }
        };
...
function showMidBossWarning(game) {
    ...
    `;
    warning.innerHTML = `
        <div style="letter-spacing: 8px;">⚠ WARNING ⚠</div>
        <div style="font-size: 24px; margin-top: 10px;">MINI BOSS APPROACHING</div>
    `;
...
function showBossWarning(game) {
    ...
    warning.style.cssText = `
        ...
        pointer-events: none;
        text-align: center;
        animation: boss-warning-pulse 0.3s infinite;
    `;
    warning.innerHTML = `
        <div style="letter-spacing: 12px;">⚠ DANGER ⚠</div>
        <div style="font-size: 36px; margin-top: 10px; color: #ffffff;">BOSS APPROACHING</div>
        <div style="font-size: 20px; margin-top: 5px; color: #ffaa00;">PREPARE FOR BATTLE</div>
    `;
