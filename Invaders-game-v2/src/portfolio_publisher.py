...
class PortfolioPublisherV2:
    ...
    def prepare_app_for_portfolio(self, app_info: Dict) -> str:
        ...
        if not self.dry_run:
            ...
            # プロフェッショナル文書をコピー（存在する場合）
            prof_docs_dir = source_path / 'professional-docs'
            if prof_docs_dir.exists():
                print("  📚 プロフェッショナル文書をコピー中...")
                shutil.copytree(prof_docs_dir,
                              os.path.join(target_dir, 'professional-docs'),
                              dirs_exist_ok=True)
                app_info['has_professional_docs'] = True
...
{self._generate_professional_docs_section(app_info)}
...
"""
    ...
    def _generate_professional_docs_section(self, app_info: Dict) -> str:
        """プロフェッショナル文書セクションを生成"""
        if not app_info.get('has_professional_docs'):
            return ""

        return """## 📚 プロフェッショナルドキュメント

このプロジェクトには、**実際の業務レベルの正式文書**も含まれています：

- 📋 [要件定義書](./professional-docs/requirements-spec.pdf)
- 📐 [基本設計書](./professional-docs/design-spec.pdf)
- 🧪 [テスト結果報告書](./professional-docs/test-report.pdf)
- 📖 [操作マニュアル](./professional-docs/user-manual.pdf)

> これらの文書は、AIエージェントにより**完全自動生成**されました。
> 実務で求められる品質の文書を、わずか数分で作成できることを実証しています。
"""
