...
- **perf:** **mp4box.js のタイムアウトを 5 秒 → 1.5 秒に短縮**（`HEAD_TIMEOUT_MS` / `TAIL_TIMEOUT_MS`）。健康な mp4 は ~50–200ms で head probe 完了するため、タイムアウト短縮は破損・非対応ファイルでの待ち時間削減に直接効く。head + tail の合計上限も 3 秒に。
- **perf:** スキャン進捗を **50 ファイル単位で console に出力**（diagnose 用）: `[scan] N/M (Xs, Y files/s)`。完了時には平均 ms/file も記録するため、ボトルネック把握が容易に。
- **improve:** **互換性スキャン中は結合 CTA を物理的に disable**（誤って解析途中で結合してしまう事故を防止）。CTA ラベルに進捗 % をライブ表示（`互換性チェック中… (37%)`）、tooltip に「互換性チェックが完了してから結合できます」を表示。
- **improve:** ユーザーが解析中に手動で選択していた場合、`finalizeMetadata` の最大グループ自動選択で **上書きしないよう保護**。`selectedFileIds.size === 0` の時のみ自動選択する defense-in-depth。
- **why:** v1.2.0 では「互換性スキャンが 500 ファイルで 5 分以上かかる」「解析中でも結合ボタンが押せてしまう」というユーザー報告。仕様の 5–15 秒に対し 20–60 倍遅かった主因は (1) 逐次プローブ + (2) 5 秒のタイムアウト。並列化 + タイムアウト短縮で根本解決。
- **既存挙動への影響:** **per-file remux + concat demuxer ロジック（v1.0.6）は完全無変更**。本リリースは互換性スキャン (Phase 1) と CTA ガード (Phase 2) のみの変更で、結合パイプラインには影響しない。
- **test:** 16 件追加（合計 307 件、全て pass）。
  - `useCompatibilityScan.test.ts` 新規 7 件: 並列度 6、キャッシュヒット時の probe スキップ、cancel 伝播、進捗更新、空ツリー処理
  - `mp4Metadata.test.ts` 3 件追加: HEAD/TAIL タイムアウトが 1.5 秒であること、合計が 3 秒以下であることの境界検証
  - `JoinActionBar.test.tsx` 4 件追加: スキャン中の disable、進捗 % 表示、tooltip 文言、スキャン完了後の再 enable
  - `appStore.test.ts` 3 件追加: `finalizeMetadata` の自動選択 / 手動選択保護 / グループなし時の挙動

## v1.2.0 (2026-05-03)

- **improve:** **互換性チェック中の進捗をメインエリアにバナー表示**。mp4box.js でメタデータを解析中、`互換性チェック中… 123 / 500 ファイル (24%)` のような進捗をプログレスバー付きで可視化。従来はファイル行の点滅バッジのみだったため、全体の文脈が見えにくかった。
- **improve:** 解析完了時に **3 秒間のサマリバナー** を表示（緑系）:「✅ 互換性チェック完了: N グループ検出。最大グループ「{label}」(M 件) を自動選択しました」。ユーザーが「何が起きたか」を一瞬で把握できるよう改善。
- **improve:** **GroupSummary パネル** に以下の機能を追加:
  - ヘッダーに「互換性グループ (N グループ + 解析失敗 M 件)」のカウント表示
  - **「🗑 全解除」ボタン**（選択あり時のみ enabled）
  - **現在選択中のグループを視覚的に強調**（インディゴの太枠 + ✅ アイコン）。「▶ こちらを基準にする」ボタンは現グループには表示されない（既に基準のため）
- **improve:** **JoinActionBar の混在時バナー** を強化:
  - 内訳行: `内訳: グループ A: 7 件 / グループ B: 3 件 / 不明: 0 件`
  - **クイックアクションボタン**: 「『グループ A』の 7 件だけ結合」「『グループ B』の 3 件だけ結合」「全解除」を 1 クリックで実行可能
  - 内訳は **大きい順** にソートされ、もっとも望ましいリスキューが左端に来る
- **why:** v1.1.1 でバージョンバッジを追加したが、ユーザーから「解析中の状況がメインビューで分かりづらい」「2 番目のグループへの切り替えや混在時のリスキューが手動で面倒」というフィードバックを受けた。可視性 + 操作柔軟性の両面を改善。
- **既存挙動への影響:** **ロジック自体（per-file remux + concat demuxer + 互換性グルーピング）は v1.1.0 と完全に同等で freeze 対策はそのまま維持**。今回の変更は UI 層（バナー、パネル、クイックアクション）に閉じている。
- **test:** 15 件追加（合計 291 件、全て pass）。
  - `MetadataScanBanner.test.tsx` 5 件: loading/summary 状態切替、進捗テキスト、3 秒タイマー、未走査時の非表示
  - `GroupSummary.test.tsx` 4 件追加: 全解除ボタンの enabled/disabled、グループ数表示、現選択中グループの強調
  - `JoinActionBar.test.tsx` 6 件追加: 混在時の内訳行、クイックアクションボタンの存在、ソート順、不明グループ表示

## v1.1.0 (2026-05-03)

- **feat:** **互換性グループ自動選択機能** を追加。
  - 起動時に MP4 ヘッダを **mp4box.js** でパースし、互換性のあるファイル群（解像度・fps・コーデック・音声仕様が一致）を自動グルーピング。
  - 最大グループのファイルが起動時に **自動選択** され、ワンクリックで結合可能。
  - 各ファイル行に色付きの **互換性バッジ**（最大グループ=`indigo`、2 番目=`emerald`、…）を表示。
  - **GroupSummary パネル** から別グループへワンクリック切替可能（「▶ こちらを基準にする」）。
  - **不互換**のファイルが選択された場合は結合ボタン無効化 + ガイダンス（「N グループ混在」など）。
  - 解析失敗ファイル（破損 / 非標準 mp4）は自動的に **不明グループ** として除外され、チェックボックスも disabled に。
  - 解析結果を **IndexedDB にキャッシュ**（`name + size + lastModified` をキー化）。再走査時は瞬時に表示。
  - 解析中も UI はブロックされない（async / await 経由 + ファイル単位の逐次反映）。
- **why:** v1.0.6 で per-file remux による edit list 起因の freeze は解消したが、**コーデックパラメータ（解像度/fps/ピクセルフォーマット/音声仕様）が異なるファイル間の境界**での freeze は `-c copy` 制約上、結合時には直せない。**結合前に互換性のあるファイルだけを選択させる UX** が根本解決策。
- **mp4box.js を選定した理由:** ffmpeg.wasm を呼ぶより圧倒的に軽量（先頭〜末尾 2 MiB の `moov` atom だけ読めば十分）。500 ファイル走査で数秒〜十数秒程度。
- **既存挙動への影響:** v1.0.6 の per-file remux + concat パイプライン（`joinDemuxer`）は **そのまま維持**。本機能は結合前の選択 UX 改善に閉じている。
- **test:** 49 件追加（合計 276 件、全て pass）。
