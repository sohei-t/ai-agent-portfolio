...
- 🛡 **skip-on-failure 戦略**（v2.1.0〜）: フォルダドロップ後は即座にツリーが見えます（事前スキャンなし）。「結合」ボタンを押すと、各ファイルの remux に **8 秒のタイムアウト** を設定して順次処理します。
  - 8 秒以内に remux 完了したファイルだけを結合する
  - タイムアウト・解析失敗・読み込み失敗のファイルは **スキップ** して残りで結合を続行する
  - 結果パネルに「N 件結合 / M 件スキップ」と、スキップ理由付きのファイル一覧を表示する
  - タイムアウト発生時は ffmpeg worker を terminate → reload するため、特定の壊れたファイルが原因で全体が固まることはありません
  - v2.0.0 の「結合時 probe + 3 択ダイアログ」は撤去しました（probe 自体がハングする環境への対策として）
