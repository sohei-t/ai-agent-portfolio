...
- 🎬 **MP4結合**: FFmpeg.wasm（マルチスレッド版）でブラウザ内エンコード
  - 入力ストリームを自動 probe し、コーデック / 解像度 / fps / 音声パラメータが揃っていれば高速な `concat demuxer (-c copy)` を使用、揃っていなければ自動的に再エンコードに切替
  - PTS 修復フラグ（`-auto_convert 1`、`-avoid_negative_ts make_zero`、`-movflags +faststart`）により結合動画途中の freeze を抑止
  - **再エンコード時は 2 段階正規化方式（v1.0.4〜）**: 各入力を `1280x720 / 30fps / yuv420p / H.264 + AAC stereo 44.1 kHz` に正規化してから concat demuxer で連結。解像度・FPS・音声仕様が揃っていない実世界の動画でも確実に結合できる。音声欠落ファイルには無音トラックを自動合成。個別ファイルの正規化失敗時は他のファイルの結合は続行
  - **エンコードモードを 2 択セグメントで選択可能（v1.0.2〜）**:
    - ⚡ **高速モード**（推奨デフォルト）: コーデック自動判定。揃っていれば高速結合
    - 🎬 **安定モード**: 常に 2 段階正規化で結合。コーデックがバラバラでも確実に動作（処理時間は増える）
