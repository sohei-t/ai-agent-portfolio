...
- 🎨 **互換性グループ自動選択**（v1.1.0〜）: 起動時に mp4box.js で各 MP4 の解像度・fps・コーデック・音声仕様を解析し、結合可能なファイル群を色付きバッジで可視化。最大グループを自動選択し、ワンクリックで結合実行。互換性のないファイルが混ざると結合ボタンを無効化してガイダンスを表示。解析結果は IndexedDB にキャッシュされるため再走査時は瞬時に表示。
