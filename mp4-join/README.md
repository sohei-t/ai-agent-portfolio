...
- 🛡 **結合時フェイルファスト互換性チェック**（v2.0.0〜）: フォルダドロップ後は即座にツリーが見えます（事前スキャンなし）。「結合」ボタンを押した時点で、選択したファイルだけを ffmpeg.wasm で逐次プローブし（典型的に数秒〜十数秒）、コーデック・解像度・fps が混ざっていればモーダルで 3 択を提示:
  - **最大グループ (N 件) だけ結合**（推奨・高速、`-c copy`）
  - **全 N 件を再エンコード**（互換性優先・低速、フィルタグラフ）
  - **キャンセル**
  すべて互換ならダイアログを出さずにそのまま結合します。
