/**
 * ROBO BATTLE V5.0 - MATCH FORMAT EDITION
 * Single-file game implementation with AI-generated robot sprites
 *
 * V5 New Features:
 * - Match Format Selection (1本先取, 2本先取, 3本先取)
 * - Multi-round battle system with win tracking
 * - Auto-continue between rounds
 * - Special ending screens (CHAMPION/GAME OVER)
 * - Victory/Defeat BGM for endings
 * - Online mode: Host controls match format
 *
 * Previous Features (V1-V4):
 * - 1P vs CPU / Online P2P battle
 * - Beast Summon System (AI-controlled beasts)
 * - Guard/Knockdown systems
 * - 11 weapons including Dragon, Meteor, Beast Summon
 * - 6 stages with unique themes
...
const GameState = {
    ...
    KO: 'ko',           // NEW: KO演出状態
    RESULT: 'result',
    ENDING: 'ending',   // V5: Special ending screen for multi-round victory
    PAUSED: 'paused',
    // Online mode states
...
const SoundManager = {
    ...
    // V5: Victory BGM (triumphant, major key)
    victoryBGMPattern: [
    ...
    // V5: Defeat BGM (melancholic, minor key)
    defeatBGMPattern: [
    ...
    playBGM(type = 'title') {
        ...
        this.stopBGM();
        // V5: Support victory and defeat BGM types
        switch (type) {
            case 'victory':
                this.bgmNotes = this.victoryBGMPattern;
                break;
            case 'defeat':
                this.bgmNotes = this.defeatBGMPattern;
                break;
            case 'battle':
                this.bgmNotes = this.battleBGMPattern;
                break;
            default:
                this.bgmNotes = this.titleBGMPattern;
        }
        this.bgmIndex = 0;
        ...
        playNext();
        // V5: Different intervals for different BGM types
        let interval = 200; // default (title)
        if (type === 'battle') interval = 150;
        else if (type === 'victory') interval = 220;
        else if (type === 'defeat') interval = 280;
        this.bgmInterval = setInterval(playNext, interval);
    },
...
class Beast {
    ...
    update(platforms) {
        ...
        // V4.9: Improved Beast AI - ALWAYS chase, prioritize same Y level
        if (this.target && !this.flameActive) {
            ...
            // V4.9: Only attack when on approximately same Y level (within 50px)
            const onSameLevel = absDistY < 50;

            if (onSameLevel && absDistX < this.flameRange && Date.now() - this.lastFlameTime > this.flameCooldown) {
                this.startFlameAttack();
            }

            // V4.9: Movement - ALWAYS move, never idle
            // Priority 1: Get on same Y level
            // Priority 2: Approach horizontally to ideal distance

            if (distY < -40 && this.onGround) {
                // Target is ABOVE - must jump!
                this.velocityY = -12;  // Strong jump
                this.onGround = false;
                this.state = 'jump';
                // Also move toward target while jumping
                this.velocityX = (distX > 0 ? 1 : -1) * this.speed * 1.2;
            } else if (onSameLevel) {
                // On same level - approach horizontally
                if (absDistX > this.idealDistance + 10) {
                    // Too far - approach target
                    this.velocityX = (distX > 0 ? 1 : -1) * this.speed;
                    this.state = 'walk';
                } else if (absDistX < this.idealDistance - 30) {
                    // Too close - back away while still attacking
                    this.velocityX = (distX > 0 ? -1 : 1) * this.speed * 0.6;
                    this.state = 'walk';
                } else {
                    // At good distance - small movements to stay active
                    this.velocityX = (distX > 0 ? 1 : -1) * this.speed * 0.3;
                    this.state = 'walk';
                }
            } else {
                // Not on same level - move toward target, will drop or need to jump
                this.velocityX = (distX > 0 ? 1 : -1) * this.speed;
                this.state = this.onGround ? 'walk' : 'jump';

                // If on ground and target is below but we're at platform edge, we'll naturally fall
                // If target is above, try jumping periodically
                if (distY < -20 && this.onGround && Math.random() < 0.05) {
                    this.velocityY = -11;
                    this.onGround = false;
                    this.state = 'jump';
                }
            }
        } else if (!this.flameActive) {
            // No target or target lost - keep moving in current direction
            this.velocityX = (this.facingRight ? 1 : -1) * this.speed * 0.5;
            this.state = 'walk';
        }
...
class EnemyAI {
    ...
    decide(player, beams, activeBeasts = [], activeItems = []) {
        ...
        // V4.9: Revised priority system for CONSTANT ACTION
        // Priority 1 (HIGHEST): Red Beast - ALWAYS attack when exists
        // Priority 2: Items - get and use immediately
        // Priority 3 (DEFAULT): Red Robot - ALWAYS chase and attack, never idle

        // Reset targets
        this.targetBeast = null;
        this.targetItem = null;

        // === PRIORITY 1 (HIGHEST): Red Beast ===
        // If player's beast exists, ALWAYS prioritize attacking it
        const enemyBeast = activeBeasts.find(b => b.active && b.owner === 'player');

        if (enemyBeast) {
            const beastDist = Math.abs(enemyBeast.x - this.robot.x);
        ...
        // === PRIORITY 2: Items ===
        // No beast exists, check for items within 350px range
        const closestItem = activeItems
            .filter(item => item && item.active)
            .sort((a, b) => Math.abs(a.x - this.robot.x) - Math.abs(b.x - this.robot.x))[0];

        if (closestItem) {
            const itemDist = Math.abs(closestItem.x - this.robot.x);
            if (itemDist < 350) {
        ...
        // === PRIORITY 3 (DEFAULT): Red Robot - ALWAYS chase and attack ===

        // Quick evade for very close beams (but still attack after)
        const incomingBeam = beams.find(b =>
            b.owner === 'player' &&
            Math.abs(b.y - this.robot.y) < 40 &&
            Math.abs(b.x - this.robot.x) < 80
        );

        if (incomingBeam) {
            // Jump to evade but continue attacking
            this.currentAction = 'jump';
            return;
        }

        // Close range (< 60px) - aggressive melee combat
        if (dist < 60) {
            const actions = ['kick', 'shoot', 'kick'];
            const weights = this.difficulty === 'hard' ? [0.5, 0.3, 0.2] : [0.4, 0.4, 0.2];
            this.currentAction = this.weightedRandom(actions, weights);
            return;
        }

        // All other ranges - ALWAYS approach and shoot (never idle)
        // Jump if player is significantly above
        if (heightDiff < -60 && Math.random() < 0.4) {
            this.currentAction = 'jump';
            return;
        }

        // Default: ALWAYS approach and shoot - never stay still
        this.currentAction = 'approach_shoot';
    }
    ...
    executeAction(player, beams, weaponProjectiles = [], activeBeasts = []) {
        ...
        switch (this.currentAction) {
            ...
            // V4.9: Approach AND shoot - includes vertical movement
            case 'approach_shoot':
                const approachDir2 = player.x > this.robot.x ? 1 : -1;
                const playerDistY = player.y - this.robot.y;  // Positive = player below
                this.robot.move(approachDir2);

                // V4.9: Jump if player is above and we're on ground
                if (playerDistY < -40 && this.robot.onGround) {
                    this.robot.jump();
                } else if (playerDistY < -20 && this.robot.onGround && Math.random() < 0.08) {
                    // Random jumps to reach higher platforms
                    this.robot.jump();
                }
            ...
            // V4.9: Get item action - chase item while ALWAYS fighting
            case 'get_item':
                if (this.targetItem && this.targetItem.active) {
                    ...
                    // V4.9: ALWAYS shoot while getting items
                    this.robot.shoot(beams);
                    if (this.robot.equippedWeapon && this.robot.equippedWeapon.type) {
                        this.robot.fireWeapon(weaponProjectiles, activeBeasts, player);
                    }
                } else {
                    // Item was picked up or disappeared - chase player instead of stopping
                    const chaseDir = player.x > this.robot.x ? 1 : -1;
                    this.robot.move(chaseDir);
                    this.robot.shoot(beams);
                }
                break;

            // V4.9: Attack beast action - ALWAYS chase and attack enemy beast
            case 'attack_beast':
                if (this.targetBeast && this.targetBeast.active) {
                    const beastDistX = Math.abs(this.targetBeast.x - this.robot.x);
                    const beastDistY = this.targetBeast.y - this.robot.y;  // Positive = beast below
                    const onSameLevel = Math.abs(beastDistY) < 50;

                    // Face the beast
                    this.robot.facingRight = this.targetBeast.x > this.robot.x;

                    // ALWAYS shoot at beast
                    this.robot.shoot(beams);
                    if (this.robot.equippedWeapon && this.robot.equippedWeapon.type) {
                        this.robot.fireWeapon(weaponProjectiles, activeBeasts, player);
                    }

                    // V4.9: Prioritize getting on same Y level, then approach
                    if (beastDistY < -40 && this.robot.onGround) {
                        // Beast is ABOVE - must jump!
                        this.robot.jump();
                        // Also move toward beast while jumping
                        const approachDir = this.targetBeast.x > this.robot.x ? 1 : -1;
                        this.robot.move(approachDir);
                    } else if (beastDistX < 50 && onSameLevel) {
                        // Very close on same level - back away slightly
                        const backDir = this.targetBeast.x > this.robot.x ? -1 : 1;
                        this.robot.move(backDir);
                    } else {
                        // Always approach - never stop
                        const approachDir = this.targetBeast.x > this.robot.x ? 1 : -1;
                        this.robot.move(approachDir);
                        // Jump periodically if beast is above
                        if (beastDistY < -20 && this.robot.onGround && Math.random() < 0.1) {
                            this.robot.jump();
                        }
                    }
                } else {
                    // Beast died - immediately chase player instead of stopping
                    const chaseDir = player.x > this.robot.x ? 1 : -1;
                    this.robot.move(chaseDir);
                    this.robot.shoot(beams);
                }
            ...
            case 'idle':
            default:
                // V4.9: Even idle/default should chase player - never stop
                const defaultChaseDir = player.x > this.robot.x ? 1 : -1;
                this.robot.move(defaultChaseDir);
                this.robot.shoot(beams);
                break;
        }
    }
...
class UIRenderer {
    ...
    drawLogo(x, y) {
        ...
        ctx.lineWidth = 1;
        ctx.fillText('V5 - MATCH FORMAT EDITION', x, y + 65);
    }
}
...
class Game {
    constructor() {
        ...
        this.settings = {
            ...
            // V5: Match Format setting (1=1本先取, 2=2本先取, 3=3本先取)
            matchFormat: 1
        };
        ...
        this.autoContinueDelay = 4000; // 4 seconds before auto-continue
        ...
        this.itemConfigScrollOffset = 0;  // V4.5: Scroll offset for item config
        this.winner = null;
    ...
    handleClick(x, y) {
        ...
        switch (this.state) {
            ...
            case GameState.ENDING:
                console.log('[DEBUG] Routing to handleEndingClick');
                this.handleEndingClick(x, y);
                break;
    ...
    handleSetupClick(x, y) {
        ...
        // V5: Calculate button selections based on Items Mode (must match renderSetup and updateSetup)
        // Shifted indices due to MATCH FORMAT at index 7
        const itemConfigSelectionIdx = 8;  // Only used when Items Mode is ON
    ...
    // V4.5: Item Config Screen click handler - Single column scrollable
    handleItemConfigClick(x, y) {
        const itemKeys = Object.keys(this.settings.enabledItems);
        const totalItems = itemKeys.length;

        // V4.7: Match render layout exactly
        const rowHeight = 65;
        const startY = 70;
        const listHeight = GAME_HEIGHT - 180;
        const visibleItems = Math.floor(listHeight / rowHeight);
        const rowWidth = GAME_WIDTH - 130;  // Leave more space for scroll buttons
        const rowX = 20;

        // V4.7: Extra-large scroll button tap areas for mobile
        const scrollBtnWidth = 100;  // Wider tap area
        const scrollBtnHeight = 100; // Taller tap area
        const scrollBtnX = GAME_WIDTH - scrollBtnWidth - 5;  // Start earlier
        const maxScroll = Math.max(0, totalItems - visibleItems);
        const upBtnY = startY;  // Start from top of list
        const downBtnY = startY + listHeight - scrollBtnHeight;  // End at bottom

        console.log(`[SCROLL DEBUG] Click at (${Math.round(x)}, ${Math.round(y)}), scrollBtnX=${scrollBtnX}, upBtnY=${upBtnY}, downBtnY=${downBtnY}, offset=${this.itemConfigScrollOffset}, maxScroll=${maxScroll}`);

        // Check scroll buttons FIRST with expanded tap area
        if (x >= scrollBtnX - 20) {  // Even more generous X check
            console.log('[SCROLL DEBUG] X is in scroll button range');
            // UP button area - top half of right side
            if (y >= upBtnY && y <= upBtnY + scrollBtnHeight + 20) {
                console.log('[SCROLL DEBUG] UP button hit!');
                // Move selection UP (this also triggers scroll in render)
                if (this.itemConfigSelection > 0) {
                    this.itemConfigSelection--;
                    SoundManager.playMenuSelect();
                    console.log('[SCROLL DEBUG] Selection moved UP to', this.itemConfigSelection);
                } else {
                    console.log('[SCROLL DEBUG] Already at top item');
                }
                return;
            }
            // DOWN button area - bottom half of right side
            if (y >= downBtnY - 20 && y <= downBtnY + scrollBtnHeight + 20) {
                console.log('[SCROLL DEBUG] DOWN button hit!');
                // Move selection DOWN (this also triggers scroll in render)
                if (this.itemConfigSelection < totalItems - 1) {
                    this.itemConfigSelection++;
                    SoundManager.playMenuSelect();
                    console.log('[SCROLL DEBUG] Selection moved DOWN to', this.itemConfigSelection);
                } else {
                    console.log('[SCROLL DEBUG] Already at bottom item');
                }
                return;
            }
            console.log('[SCROLL DEBUG] Y not in button range');
        }

        // V4.5: Check single column items (with scroll offset)
        for (let i = 0; i < visibleItems && i + this.itemConfigScrollOffset < totalItems; i++) {
            const itemIndex = i + this.itemConfigScrollOffset;
            const itemY = startY + i * rowHeight;
            if (x >= rowX && x <= rowX + rowWidth && y >= itemY && y <= itemY + 58) {
                this.itemConfigSelection = itemIndex;
        ...
        // V4.5: Button layout - larger for mobile (must match render)
        ...
        // BACK button
        if (x >= btnStartX + (btnWidth + btnGap) * 2 && x <= btnStartX + btnWidth * 3 + btnGap * 2 && y >= btnY && y <= btnY + btnHeight) {
    ...
    updateSetup(deltaTime) {
        ...
        if (!this.isOnlineMode || !this.myReady) {
            // V5: Reorganized navigation order (added MATCH FORMAT at index 7)
            // 0-6: Parameters and settings
            // 7: MATCH FORMAT
            // 8: ITEM CONFIG (when Items Mode ON) or START BATTLE (when OFF)
            // 9: START BATTLE (when Items Mode ON) or BACK (when OFF)
            // 10: BACK (when Items Mode ON)
            const maxSelection = this.settings.itemsMode ? 10 : 9;
            ...
            // V5: Adjust values (now for items 0-7 including MATCH FORMAT)
            if (this.setupSelection <= 7) {
        ...
        // V5: ITEM CONFIG button (selection 8 when Items Mode is ON, shifted from 7)
        if (this.settings.itemsMode && this.setupSelection === 8 &&
        ...
        // V5: Start battle / Ready button
        // Selection 9 when Items Mode ON, Selection 8 when OFF (shifted from 8/7)
        const startButtonSelection = this.settings.itemsMode ? 9 : 8;
    ...
    adjustSetup(direction) {
        ...
        switch (this.setupSelection) {
            ...
            case 6: // Items Mode
                ...
                break;
            case 7: // V5: Match Format
                // Cycle through 1, 2, 3 (1本先取, 2本先取, 3本先取)
                if (direction > 0) {
                    this.settings.matchFormat = this.settings.matchFormat >= 3 ? 1 : this.settings.matchFormat + 1;
                } else {
                    this.settings.matchFormat = this.settings.matchFormat <= 1 ? 3 : this.settings.matchFormat - 1;
                }
                break;
        }
    }
    ...
    startBattle() {
        ...
        // V5: Reset match tracking for new game
        this.playerWins = 0;
        this.cpuWins = 0;
        this.currentRound = 1;
        this.matchDecided = false;
        this.autoContinueTimer = 0;
        console.log(`[V5] New match started - Format: ${this.settings.matchFormat}本先取`);
    ...
    // V5: Start the next round in a multi-round match (keeps score, resets positions)
    startNextRound() {
        console.log(`[V5] startNextRound() called - Round ${this.currentRound}`);

        const stage = STAGES[this.currentStage];

        // Reset robot positions and HP (keep same parameters)
        this.player.x = stage.spawnPoints.player.x;
        this.player.y = stage.spawnPoints.player.y;
        ...
        this.player.weaponAmmo = 0;
        this.player.isGuarding = false;
        this.player.knockdownTimer = 0;

        this.enemy.x = stage.spawnPoints.enemy.x;
        this.enemy.y = stage.spawnPoints.enemy.y;
        ...
        this.enemy.chargeLevel = 0;
        this.enemy.currentWeapon = null;
        this.enemy.weaponAmmo = 0;
        this.enemy.isGuarding = false;
        this.enemy.knockdownTimer = 0;

        // Re-initialize AI
        if (!this.isOnlineMode && this.ai) {
            this.ai = new EnemyAI(this.enemy);
            this.ai.setDifficulty(this.settings.difficulty);
        }
        ...
        console.log(`[V5] Round ${this.currentRound} started!`);
    }
    ...
    startOnlineBattle() {
        ...
        this.onlineController.sendBothReady({
            ...
            matchFormat: this.settings.matchFormat,    // V5: Host's match format
    ...
    updateResult(deltaTime) {
        ...
        // V5: Auto-continue for multi-round matches (not decided yet, offline only)
        const isMultiRound = this.settings.matchFormat > 1;
        if (!this.isOnlineMode && isMultiRound && !this.matchDecided && this.autoContinueTimer > 0) {
            const elapsed = Date.now() - this.autoContinueTimer;

            // Check for skip input or timer expired
            if (elapsed >= this.autoContinueDelay ||
                input.shoot || input.jump || this.input.keys['Enter'] || this.input.keys['Space']) {

                // Start next round
                console.log(`[V5] Starting Round ${this.currentRound + 1}`);
                this.currentRound++;
                this.autoContinueTimer = 0;
                this.startNextRound();
                this.inputCooldown = 200;
            ...
            return; // Don't process menu input during auto-continue
        }
    ...
    updateKO(deltaTime) {
        ...
        if (!isOnlineClient && this.koTimer >= KO_SETTINGS.freezeTime) {
            // V5: Track wins for multi-round matches
            if (this.winner === 'player') {
                this.playerWins++;
            } else {
                this.cpuWins++;
            }
            console.log(`[V5] Score: Player ${this.playerWins} - ${this.cpuWins} CPU (Round ${this.currentRound})`);

            // V5: Check if match is decided
            const winsNeeded = this.settings.matchFormat;
            this.matchDecided = (this.playerWins >= winsNeeded || this.cpuWins >= winsNeeded);

            // V5: Initialize auto-continue timer for multi-round matches
            if (this.settings.matchFormat > 1 && !this.matchDecided) {
                this.autoContinueTimer = Date.now();
            } else {
                this.autoContinueTimer = 0;
            }

            // V5: Go to ENDING screen for decided multi-round matches, else RESULT
            if (this.settings.matchFormat > 1 && this.matchDecided) {
                this.state = GameState.ENDING;
                this.menuSelection = 0;
                // Play ending BGM
                SoundManager.playBGM(this.playerWins >= winsNeeded ? 'victory' : 'defeat');
                console.log('[V5] 演出終了 → ENDING画面へ');
            } else {
    ...
    renderSetup() {
        ...
        // V5: Calculate button selections based on Items Mode (shifted by 1 for MATCH FORMAT)
        const itemConfigSelection = 8;  // Only used when Items Mode is ON (shifted from 7)
        const startButtonSelection = this.settings.itemsMode ? 9 : 8;
        const backButtonSelection = this.settings.itemsMode ? 10 : 9;
    ...
    // V4.5: Item Configuration Screen - Single column scrollable layout with large text
    renderItemConfig() {
        const ctx = this.ctx;
        ...
        ctx.shadowBlur = 15;
        ctx.fillText('⚙ ITEM CONFIG', GAME_WIDTH / 2, 40);
        ctx.shadowBlur = 0;
        ...
        // V4.5: Single column scrollable layout
        const rowHeight = 65;  // Larger rows for easy tapping
        const startY = 70;
        const listHeight = GAME_HEIGHT - 180;  // Space for buttons at bottom
        const visibleItems = Math.floor(listHeight / rowHeight);  // ~6-7 items visible

        // Auto-scroll to keep selected item visible
        if (this.itemConfigSelection < totalItems) {
            if (this.itemConfigSelection < this.itemConfigScrollOffset) {
                this.itemConfigScrollOffset = this.itemConfigSelection;
            } else if (this.itemConfigSelection >= this.itemConfigScrollOffset + visibleItems) {
                this.itemConfigScrollOffset = this.itemConfigSelection - visibleItems + 1;
            }
        }

        // Clamp scroll offset
        const maxScroll = Math.max(0, totalItems - visibleItems);
        this.itemConfigScrollOffset = Math.max(0, Math.min(this.itemConfigScrollOffset, maxScroll));

        // V4.5: Item display info with large text
        const itemDisplayInfo = {
            HP: { name: 'HP回復', color: '#ff6666', desc: '+50HP回復' },
            RAPID: { name: '連射UP', color: '#ffff00', desc: '攻撃速度2倍' },
            MEGA: { name: 'メガ弾', color: '#00ff00', desc: 'ダメージ2.5倍' },
            SHIELD: { name: 'シールド', color: '#00ffff', desc: '一定時間無敵' },
            W_BAZOOKA: { name: 'バズーカ', color: '#ff6600', desc: '爆発攻撃' },
            W_MACHINEGUN: { name: 'マシンガン', color: '#ffff00', desc: '高速連射' },
            W_SPREAD: { name: 'スプレッド', color: '#00ff99', desc: '3方向攻撃' },
            W_SWORD: { name: '斬撃', color: '#ff00ff', desc: '近接高威力' },
            W_HOMING: { name: '誘導弾', color: '#ff3333', desc: '敵を自動追尾' },
            W_CLONE: { name: '分身', color: '#9933ff', desc: '撹乱用分身' },
            W_TIGER: { name: '虎召喚', color: '#ff9900', desc: '虎が敵を攻撃' },
            W_AERIAL: { name: '空爆', color: '#00ffff', desc: '空から4発攻撃' },
            W_DRAGON: { name: '龍召喚', color: '#00ff66', desc: 'ドラゴン火炎' },
            W_METEOR: { name: 'メテオ', color: '#ff6600', desc: '流星群攻撃' },
            W_BEAST: { name: '魔獣召喚', color: '#ff33ff', desc: 'AI魔獣が追尾' }
        };
        ...
        for (let i = 0; i < visibleItems && i + this.itemConfigScrollOffset < totalItems; i++) {
            const itemIndex = i + this.itemConfigScrollOffset;
            const key = itemKeys[itemIndex];
            const isSelected = this.itemConfigSelection === itemIndex;
            const isEnabled = this.settings.enabledItems[key];
            const info = itemDisplayInfo[key] || { name: key, color: '#ffffff', desc: '' };
            const y = startY + i * rowHeight;

            this.drawItemConfigRowLarge(ctx, y, key, info, isSelected, isEnabled);
        }

        // V4.6: Large scroll buttons on right side
        if (totalItems > visibleItems) {
            const scrollBtnWidth = 70;
            const scrollBtnHeight = 60;
            const scrollBtnX = GAME_WIDTH - scrollBtnWidth - 10;

            // Scroll position indicator text
            ctx.font = '14px Courier New';
            ctx.fillStyle = '#888888';
            ctx.textAlign = 'center';
            ctx.fillText(`${this.itemConfigScrollOffset + 1}-${Math.min(this.itemConfigScrollOffset + visibleItems, totalItems)}/${totalItems}`,
                scrollBtnX + scrollBtnWidth/2, startY + listHeight/2);

            // UP button - large and tappable
            const upBtnY = startY + 20;
            const canScrollUp = this.itemConfigScrollOffset > 0;
            ctx.fillStyle = canScrollUp ? '#660066' : '#222222';
            ctx.fillRect(scrollBtnX, upBtnY, scrollBtnWidth, scrollBtnHeight);
            ctx.strokeStyle = canScrollUp ? '#ff00ff' : '#444444';
            ctx.lineWidth = 3;
            ctx.strokeRect(scrollBtnX, upBtnY, scrollBtnWidth, scrollBtnHeight);
            ctx.font = 'bold 24px Courier New';
            ctx.fillStyle = canScrollUp ? '#ff00ff' : '#444444';
            ctx.textAlign = 'center';
            ctx.fillText('▲', scrollBtnX + scrollBtnWidth/2, upBtnY + 28);
            ctx.font = 'bold 14px Courier New';
            ctx.fillText('UP', scrollBtnX + scrollBtnWidth/2, upBtnY + 48);

            // DOWN button - large and tappable
            const downBtnY = startY + listHeight - scrollBtnHeight - 20;
            const canScrollDown = this.itemConfigScrollOffset < maxScroll;
            ctx.fillStyle = canScrollDown ? '#660066' : '#222222';
            ctx.fillRect(scrollBtnX, downBtnY, scrollBtnWidth, scrollBtnHeight);
            ctx.strokeStyle = canScrollDown ? '#ff00ff' : '#444444';
            ctx.lineWidth = 3;
            ctx.strokeRect(scrollBtnX, downBtnY, scrollBtnWidth, scrollBtnHeight);
            ctx.font = 'bold 24px Courier New';
            ctx.fillStyle = canScrollDown ? '#ff00ff' : '#444444';
            ctx.textAlign = 'center';
            ctx.fillText('▼', scrollBtnX + scrollBtnWidth/2, downBtnY + 28);
            ctx.font = 'bold 14px Courier New';
            ctx.fillText('DOWN', scrollBtnX + scrollBtnWidth/2, downBtnY + 48);
        }

        // Buttons at bottom - larger for mobile
        const btnY = GAME_HEIGHT - 95;
        const btnWidth = 130;
        const btnHeight = 50;
        const btnGap = 15;
        ...
        ctx.fillRect(btnStartX, btnY, btnWidth, btnHeight);
        ctx.strokeStyle = allOnSelected ? '#00ff00' : '#666666';
        ctx.lineWidth = 3;
        ctx.strokeRect(btnStartX, btnY, btnWidth, btnHeight);
        ctx.font = allOnSelected ? 'bold 18px Courier New' : '18px Courier New';
        ctx.fillStyle = '#00ff00';
        ctx.textAlign = 'center';
        ctx.fillText('ALL ON', btnStartX + btnWidth/2, btnY + 32);

        // ALL OFF button
        const allOffSelected = this.itemConfigSelection === totalItems + 1;
        ctx.fillStyle = allOffSelected ? '#660000' : '#333333';
        ctx.fillRect(btnStartX + btnWidth + btnGap, btnY, btnWidth, btnHeight);
        ctx.strokeStyle = allOffSelected ? '#ff0000' : '#666666';
        ctx.strokeRect(btnStartX + btnWidth + btnGap, btnY, btnWidth, btnHeight);
        ctx.font = allOffSelected ? 'bold 18px Courier New' : '18px Courier New';
        ctx.fillStyle = '#ff6666';
        ctx.fillText('ALL OFF', btnStartX + btnWidth + btnGap + btnWidth/2, btnY + 32);

        // BACK button
        const backSelected = this.itemConfigSelection === totalItems + 2;
        ctx.fillStyle = backSelected ? '#444444' : '#222222';
        ctx.fillRect(btnStartX + (btnWidth + btnGap) * 2, btnY, btnWidth, btnHeight);
        ctx.strokeStyle = backSelected ? '#ffffff' : '#666666';
        ctx.strokeRect(btnStartX + (btnWidth + btnGap) * 2, btnY, btnWidth, btnHeight);
        ctx.font = backSelected ? 'bold 18px Courier New' : '18px Courier New';
        ctx.fillStyle = '#ffffff';
        ctx.fillText('BACK', btnStartX + (btnWidth + btnGap) * 2 + btnWidth/2, btnY + 32);
        ...
        ctx.textAlign = 'center';
        ctx.fillText(`${enabledCount}/${totalItems} ON`, GAME_WIDTH / 2, GAME_HEIGHT - 15);
    }

    // V4.7: Large item row for scrollable list (with space for scroll buttons)
    drawItemConfigRowLarge(ctx, y, key, info, isSelected, isEnabled) {
        const rowWidth = GAME_WIDTH - 130;  // Leave more space for scroll buttons on right
        const rowHeight = 58;
        const x = 20;
        ...
        // Item color indicator (large circle)
        ctx.beginPath();
        ctx.arc(x + 30, y + rowHeight/2, 16, 0, Math.PI * 2);
        ctx.fillStyle = isEnabled ? info.color : '#333333';
        ctx.fill();
        ...
        // Item name - LARGE
        ctx.font = isSelected ? 'bold 22px Courier New' : '22px Courier New';
        ctx.fillStyle = isSelected ? '#ffffff' : (isEnabled ? '#dddddd' : '#666666');
        ctx.textAlign = 'left';
        ctx.fillText(info.name, x + 60, y + 28);

        // Description - smaller but still readable
        ctx.font = '16px Courier New';
        ctx.fillStyle = isEnabled ? '#888888' : '#555555';
        ctx.fillText(info.desc, x + 60, y + 50);

        // ON/OFF indicator - large and clear
        ctx.font = isSelected ? 'bold 20px Courier New' : '18px Courier New';
        ctx.textAlign = 'right';
        if (isEnabled) {
            ctx.fillStyle = '#00ff00';
            ctx.fillText('● ON', x + rowWidth - 15, y + 38);
        } else {
            ctx.fillStyle = '#666666';
            ctx.fillText('○ OFF', x + rowWidth - 15, y + 38);
        }
    }
    ...
    renderBattle() {
        ...
        // =====================================================
        // BATTLE COUNTDOWN DISPLAY: 3-2-1-FIGHT (V5: with round info)
        // =====================================================
        if (this.battleCountdownStart > 0) {
            ...
            if (countdownValue > 0 || elapsed < 3800) {
                ...
                if (countdownValue > 0) {
                    ...
                    // V5: Show round info during countdown (only for multi-round matches)
                    if (this.settings.matchFormat > 1) {
                        // Match format message
                        ctx.font = 'bold 24px Courier New';
                        ctx.fillStyle = '#ff6600';
                        ctx.shadowColor = '#ff3300';
                        ctx.shadowBlur = 10;
                        const formatMsg = this.settings.matchFormat === 2 ? '先に2勝した方が勝ち！' : '先に3勝した方が勝ち！';
                        ctx.fillText(formatMsg, GAME_WIDTH / 2, 60);
                        ...
                        ctx.shadowBlur = 15;
                        ctx.fillText(`ROUND ${this.currentRound}`, GAME_WIDTH / 2, 110);

                        // Current score if not first round
                        if (this.playerWins > 0 || this.cpuWins > 0) {
                            ctx.font = 'bold 20px Courier New';
                            ctx.fillStyle = '#ffffff';
                            ctx.shadowBlur = 5;
                            ctx.fillText(`あなた ${this.playerWins} - ${this.cpuWins} CPU`, GAME_WIDTH / 2, 150);
                        }
                    }
                }
    ...
    renderResult() {
        ...
        // V5: Check if this is a multi-round match and if it's decided
        const isMultiRound = this.settings.matchFormat > 1;
        const isPlayerWinner = this.winner === 'player';

        // V5: Different title based on match status
        ctx.font = 'bold 48px Courier New';
        ctx.textAlign = 'center';
        ctx.strokeStyle = '#ffffff';
        ctx.lineWidth = 2;

        if (isMultiRound && this.matchDecided) {
            // Final victory/defeat
            const finalPlayerWin = this.playerWins >= this.settings.matchFormat;
            ctx.fillStyle = finalPlayerWin ? '#ffcc00' : '#6666ff';
            const msg = finalPlayerWin ? 'VICTORY!' : 'DEFEAT...';
            ctx.fillText(msg, GAME_WIDTH / 2, 80);
            ctx.strokeText(msg, GAME_WIDTH / 2, 80);

            // Final score
            ctx.font = 'bold 28px Courier New';
            ctx.fillStyle = '#ffffff';
            ctx.fillText(`最終結果: あなた ${this.playerWins} - ${this.cpuWins} CPU`, GAME_WIDTH / 2, 120);
        } else if (isMultiRound) {
            // Round result (not decided yet)
            ctx.fillStyle = isPlayerWinner ? '#ff3333' : '#3333ff';
            const roundMsg = isPlayerWinner ? 'ROUND WIN!' : 'ROUND LOSE...';
            ctx.fillText(roundMsg, GAME_WIDTH / 2, 80);
            ctx.strokeText(roundMsg, GAME_WIDTH / 2, 80);

            // Current score
            ctx.font = 'bold 28px Courier New';
            ctx.fillStyle = '#ffffff';
            ctx.fillText(`あなた ${this.playerWins} - ${this.cpuWins} CPU`, GAME_WIDTH / 2, 120);
        } else {
        ...
        // V5: Auto-continue for multi-round matches (not decided yet)
        if (isMultiRound && !this.matchDecided && this.autoContinueTimer > 0) {
            const elapsed = Date.now() - this.autoContinueTimer;
            const remaining = Math.max(0, Math.ceil((this.autoContinueDelay - elapsed) / 1000));

            // Auto-continue countdown
            ctx.font = 'bold 24px Courier New';
            ctx.fillStyle = '#00ffff';
            ctx.fillText(`次のラウンドまで ${remaining}...`, GAME_WIDTH / 2, 430);
            ...
            const barY = 450;
            const progress = Math.min(1, elapsed / this.autoContinueDelay);
            ...
            ctx.font = '18px Courier New';
            ctx.fillStyle = '#888888';
            ctx.fillText(`ROUND ${this.currentRound + 1} 準備中...`, GAME_WIDTH / 2, 500);

            // Skip option
            ctx.font = '14px Courier New';
            ctx.fillStyle = '#666666';
            ctx.fillText('Enter / タップ でスキップ', GAME_WIDTH / 2, 540);
            return;
        }

        // Menu (for single round or decided matches)
    ...
    // ========================================================================
    // V5: ENDING SCREEN METHODS (for multi-round match victory)
    // ========================================================================

    renderEnding() {
        const ctx = this.ctx;
        const isPlayerWinner = this.playerWins >= this.settings.matchFormat;

        // Animated background gradient
        const time = Date.now() / 1000;
        const gradient = ctx.createLinearGradient(0, 0, GAME_WIDTH, GAME_HEIGHT);
        if (isPlayerWinner) {
            // Victory: Gold/Red gradient
            gradient.addColorStop(0, '#1a0a00');
            gradient.addColorStop(0.5, '#331100');
            gradient.addColorStop(1, '#1a0500');
        } else {
            // Defeat: Blue/Purple gradient
            gradient.addColorStop(0, '#000a1a');
            gradient.addColorStop(0.5, '#001133');
            gradient.addColorStop(1, '#050019');
        }
        ctx.fillStyle = gradient;
        ctx.fillRect(0, 0, GAME_WIDTH, GAME_HEIGHT);

        // Particle effect (stars/sparks)
        ctx.save();
        for (let i = 0; i < 50; i++) {
            const x = (Math.sin(time * 0.5 + i * 0.5) * 0.5 + 0.5) * GAME_WIDTH;
            const y = (Math.cos(time * 0.3 + i * 0.7) * 0.5 + 0.5) * GAME_HEIGHT;
            const size = Math.sin(time * 2 + i) * 2 + 3;
            ctx.globalAlpha = Math.sin(time + i) * 0.3 + 0.3;
            ctx.fillStyle = isPlayerWinner ? '#ffcc00' : '#6699ff';
            ctx.beginPath();
        ...
        ctx.fillStyle = '#ffffff';
        ctx.textAlign = 'center';
        ctx.fillText(`最終スコア: ${this.playerWins} - ${this.cpuWins}`, GAME_WIDTH / 2, 180);

        // Match format info
        ctx.font = '20px Courier New';
        ctx.fillStyle = '#888888';
        ctx.fillText(`${this.settings.matchFormat}本先取マッチ`, GAME_WIDTH / 2, 210);
        ...
        ctx.save();
        ctx.shadowColor = isPlayerWinner ? '#ffcc00' : '#6699ff';
        ctx.shadowBlur = 40;
        if (winnerSprite) {
            if (isPlayerWinner) {
                ctx.drawImage(winnerSprite, winnerX, robotY, robotWidth, robotHeight);
        ...
        // Celebration text (player win)
        if (isPlayerWinner) {
            ctx.font = 'bold 24px Courier New';
            ctx.fillStyle = '#ffcc00';
            ctx.fillText('おめでとう！あなたの勝利です！', GAME_WIDTH / 2, 510);
        } else {
            ctx.font = 'bold 24px Courier New';
            ctx.fillStyle = '#6699ff';
            ctx.fillText('CPUの勝利...また挑戦しよう！', GAME_WIDTH / 2, 510);
        }

        // Menu options
        const menuItems = ['NEW MATCH', 'CHANGE SETTINGS', 'TITLE'];
        const startY = 550;
        ...
        for (let i = 0; i < menuItems.length; i++) {
            ...
            ctx.font = isSelected ? 'bold 20px Courier New' : '18px Courier New';
            ctx.fillStyle = isSelected ? '#ffff00' : '#888888';
            ctx.fillText((isSelected ? '> ' : '  ') + menuItems[i], GAME_WIDTH / 2, startY + i * 35);
        }
    }
    ...
    handleEndingClick(x, y) {
        // Menu items
        const menuItems = ['NEW MATCH', 'CHANGE SETTINGS', 'TITLE'];
        const startY = 550;

        for (let i = 0; i < menuItems.length; i++) {
            const itemY = startY + i * 35;
            if (y >= itemY - 15 && y <= itemY + 15 && x >= GAME_WIDTH / 2 - 100 && x <= GAME_WIDTH / 2 + 100) {
                this.menuSelection = i;
                SoundManager.playMenuSelect();
                switch (i) {
                    case 0: // New Match
    ...
    setupOnlineCallbacks() {
        ...
        this.onBothReady = (data) => {
            ...
            // V5: Apply host's match format
            if (data.matchFormat !== undefined) {
                this.settings.matchFormat = data.matchFormat;
                console.log('[Online] Applied host match format:', data.matchFormat);
            }
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    console.log('📅 Build: 2026-01-10 V5 MATCH FORMAT EDITION');
